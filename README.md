...
```bash
TCB_Test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
TCB_Test$ sam local start-api
TCB_Test$ curl http://localhost:3000/
```
...
```bash
TCB_Test$ sam logs -n HelloWorldFunction --stack-name TCB_Test --tail
```
...
```bash
aws cloudformation delete-stack --stack-name TCB_Test
```
